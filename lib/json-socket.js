...
JsonSocket.sendSingleMessage = function(port, host, message, opts = {}) {
    return new Promise((res, rej) => {
        var socket = new JsonSocket(new net.Socket(), opts);
        socket.connect(port, host);
        socket.on('error', rej);
        socket.on('connect', function() {
            res(socket.sendEndMessage(message));
        });
    });
};

JsonSocket.sendSingleMessageAndReceive = function(port, host, message, opts = {}) {
    return new Promise((res, rej) => {
        var socket = new JsonSocket(new net.Socket(), opts);
        socket.connect(port, host);
        socket.on('error', rej);
        socket.on('connect', function() {
            socket.sendMessage(message).catch(() => {
                socket.end();
                rej(err);
            }).then(() => {
                socket.on('message', function(message) {
                    ...
                    if (message.success === false) {
                        return rej(new Error(message.error));
                    }
                    res(message)
                });
            });
...
JsonSocket.prototype = {
    ...
    sendError: function(err) {
        return this.sendMessage(this._formatError(err));
    },
    sendEndError: function(err) {
        return this.sendEndMessage(this._formatError(err));
    },
    ...
    sendMessage: function(message) {
        return new Promise((res, rej) => {
            if (this._closed) {
                rej(new Error('The socket is closed.'));
                return;
            }
            this._socket.write(this._formatMessageData(message), 'utf-8', res);
        });
    },
    sendEndMessage: async function(message) {
        let err = null;
        try {
